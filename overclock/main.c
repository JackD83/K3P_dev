...
#define TITLE_MSG "jz4760 Overclock"
...
void draw_background(void)
{
  ...
  rt.w = 320 - 50;
  rt.h = 240 - 50;
  SDL_FillRect(ScreenSurface, &rt, SDL_MapRGB(ScreenSurface->format, 0, 0, 0));
  rt.x = 30;
  rt.y = 30;
  rt.w = 320 - 50;
  rt.h = 240 - 50;
  SDL_FillRect(ScreenSurface, &rt, SDL_MapRGB(ScreenSurface->format, 0xa0, 0xa0, 0xa0));
}
...
void draw_msg(int m)
{
  ...
  rt.x = 70;
  rt.y = 8;
  SDL_BlitSurface(msg, NULL, ScreenSurface, &rt);
  SDL_FreeSurface(msg);
...
int main(int argc, char* argv[])
{
  ...
  // background
  SDL_FillRect(ScreenSurface, &ScreenSurface->clip_rect, SDL_MapRGB(ScreenSurface->format, 0x00, 0x00, 0xc0));
  draw_msg(m);
	while(loop){
